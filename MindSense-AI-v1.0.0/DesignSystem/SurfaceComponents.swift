...
struct MindSenseBottomActionDock<Content: View>: View {
    ...
    var body: some View {
        ...
        .frame(maxWidth: .infinity, alignment: compactTabBarPresentation ? .leading : .center)
        .padding(.horizontal, dockHorizontalInset)
    ...
    private var dockHorizontalInset: CGFloat {
        compactTabBarPresentation ? MindSenseLayout.pageHorizontal : MindSenseSpacing.sm
    }
    ...
    private var dockStroke: some View {
        RoundedRectangle(cornerRadius: dockCornerRadius, style: .continuous)
            .stroke(MindSensePalette.strokeStrong.opacity(compactTabBarPresentation ? 0.16 : 0.12), lineWidth: 1)
    }
}
